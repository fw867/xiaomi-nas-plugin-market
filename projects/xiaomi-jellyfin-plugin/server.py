"""Owner-authenticated Jellyfin console. No generic Docker proxy."""
...
STATIC = {
    ...
    '/assets/jellyfin.png': ('assets/jellyfin.png', 'image/png'),
}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def session(self):
        token = self.headers.get('X-Jellyfin-Session', '')
        try:
    ...
    def do_POST(self):
        ...
        try:
            ...
            if action == 'setup':
                media = data.get('path', '')
                config = data.get('configPath', '')
                if not isinstance(media, str) or len(media) > 1024:
                    raise Error('目录路径无效')
                if not isinstance(config, str) or len(config) > 1024:
                    raise Error('配置目录路径无效')
                self.server.engine.launch('setup', {'path': media, 'configPath': config})
...
def main():
    ...
    data = os.environ.get('DATA_DIR') or (
        tempfile.mkdtemp(prefix='jellyfin-preview-') if args.dev else '/data/plugin/jellyfin/data')
    ...
    if args.stop_owned:
        ...
        return
    server = Server(('127.0.0.1', int(os.environ.get('PORT', 18170))), engine, user, args.dev)
    if engine.config and engine.config.get('enabled') and not args.dev:
        engine.launch('start', {})
    print('Jellyfin plugin listening on http://127.0.0.1:' + str(server.server_port), flush=True)
    try:
